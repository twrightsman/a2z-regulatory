...
data_dir="../../data/marand_2021_scATAC"
...
WINDOW_SIZE=$(jq --raw-output '.preprocessing.window_size' < "$PREPROCESSING_CONFIG")
...
TMP="tmp"
IDX="${data_dir}/AGPv4.fa.fai"

# process original scATAC peaks
echo "Processing original scATAC peaks..."
tail -n+2 "${data_dir}/GSE155178_maize_scATAC_atlas_ACR_celltype_CPM.txt" | \
  cut -f1 | \
  tr '_' $'\t' \
  > "$TMP"/scATAC.bed

INTERVAL_FILE="tmp/scATAC.bed"
SPECIES="scATAC"
